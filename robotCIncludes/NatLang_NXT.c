...
void updateMotorDriveTrain()
{
	...
	{
		memset(internalLeftMotors, 0xFF, sizeof(internalLeftMotors));
		memset(internalRightMotors, 0xFF, sizeof(internalRightMotors));
...
void setMotorTarget(tMotor nMotorIndex, float nPosition, short nSpeed)
{
	nMotorEncoderTarget[nMotorIndex] = nPosition;
	motor[nMotorIndex] = nSpeed;
}
...
void resetMotorEncoder(tMotor nMotorIndex)
{
	nMotorEncoder[nMotorIndex] = 0;
}
