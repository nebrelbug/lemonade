...
void lowLevelMoveTiming(float quantity, const short nLeftSpeed, const short nRightSpeed, const tMovementUnits nUnitType)
{
	...
	if(internalLeftMotors[3] != noMotor)	setMotorSpeed(internalLeftMotors[3], nLeftSpeed);
	...
	if(internalRightMotors[3] != noMotor)	setMotorSpeed(internalRightMotors[3], nRightSpeed);
	...
	if(internalLeftMotors[3] != noMotor)	setMotorSpeed(internalLeftMotors[3], 0);
	...
	if(internalRightMotors[3] != noMotor)	setMotorSpeed(internalRightMotors[3], 0);
...
void tankControl(TJoystickAxis rightJoystick = JoyY2, TJoystickAxis leftJoystick = JoyY1, short threshold = 10)
{
	...
	if (abs(nVirtualRightJoystick) <= abs(threshold))
		...
		rightSpeed = nVirtualRightJoystick;

	if (abs(nVirtualLeftJoystick) <= abs(threshold))
		leftSpeed = 0;
	else
		leftSpeed = nVirtualLeftJoystick;
